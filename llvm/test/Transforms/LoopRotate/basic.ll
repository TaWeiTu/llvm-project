...
; RUN: opt -S -passes='require<targetir>,require<assumptions>,loop-nest(loop(loop-rotate))' < %s | FileCheck %s
; RUN: opt -S -passes='require<targetir>,require<assumptions>,loop-nest(loop-mssa(loop-rotate))' -verify-memoryssa  < %s | FileCheck %s
